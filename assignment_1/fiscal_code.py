...
def generate_fcode(p):
    ...
    first_name = str(p.name).split(sep=' ')[0]
    # Last names can have more than one word
    last_name = str(p.name)[len(first_name):].replace("'", "").replace(" ", "")
    ...
    year_code = str(p.bday.year)[-2:]
    month_code = month_mapping[p.bday.month]
    day_code = '{:02d}'.format(p.bday.day + (40 if p.gender == 'F' else 0))
